...
import {
	...
	permissionsCreateUseCase
} from '$lib/server/application/use_cases/permissions/create'
...
export const permissionsCreateController = async ({
	data,
	session,
	configuration
}: {
	...
	const [errors, permissions] = await permissionsCreateUseCase({
		data,
		...getServerContext({ session, configuration })
	})
	if (errors !== null) {
